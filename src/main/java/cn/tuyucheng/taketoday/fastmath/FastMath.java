...
///
/// All methods are thread-safe and have no side effects.
///
/// Precision: Most functions have relative error less than 1e-6 or better.
public final class FastMath {
    ...
    public static float pow(float base, float exponent) {
        ...
        switch (base) {
            case 0.0f -> {
                return exponent > 0.0f ? 0.0f : Float.POSITIVE_INFINITY;
            }
            case float v when v < 0.0f && exponent != (int) exponent -> {
                return Float.NaN;
            }
            case float v when v < 0.0f -> {
    ...
    /// Fast absolute value for double precision.
    ///
    /// @param x the value
    /// @return |x|
